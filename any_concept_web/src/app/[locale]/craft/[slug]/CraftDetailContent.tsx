...
import { getCraftDetailSync, getAllCraftsSync } from '@/data/localizedData';

export default function CraftDetailContent() {
  const params = useParams();
  const t = useTranslations();
  const locale = useLocale();
  const slug = params?.slug as string;

  const craftData = useMemo(
    () => getCraftDetailSync(locale, slug),
    [locale, slug]
  );

  const allCrafts = useMemo(() => getAllCraftsSync(locale), [locale]);

  const { prevCraft, nextCraft } = useMemo(() => {
    if (!craftData) return { prevCraft: null, nextCraft: null };
    const internalCrafts = allCrafts.filter(
      (craft) => !craft.externalLink
    );
    const currentIndex = internalCrafts.findIndex((c) => c.slug === slug);
    return {
      prevCraft: currentIndex > 0 ? internalCrafts[currentIndex - 1] : null,
      nextCraft:
        currentIndex < internalCrafts.length - 1
          ? internalCrafts[currentIndex + 1]
          : null,
    };
  }, [allCrafts, craftData, slug]);
